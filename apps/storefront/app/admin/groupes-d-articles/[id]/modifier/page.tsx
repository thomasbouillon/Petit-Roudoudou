...
type EditArticleGroupForm = z.infer<typeof schema>;

export default function Page({ params: { id } }: Props) {
  const artileGroupQuery = trpc.articleGroups.findById.useQuery(id);
  const form = useForm<EditArticleGroupForm>({
    resolver: zodResolver(schema),
    values: {
      name: artileGroupQuery.data?.name ?? '',
      seo: {
        title: artileGroupQuery.data?.seo?.title ?? '',
        description: artileGroupQuery.data?.seo?.description ?? '',
      },
    },
  });

  const trpcUtils = trpc.useUtils();
  const editMutation = trpc.articleGroups.update.useMutation({
    onSuccess: () => {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    await editMutation
      ...
      .then(() => {
        router.push(routes().admin().articleGroups().index());
      })
      .catch((error) => {
        console.error(error);
  ...
  return (
    <div className="max-w-md mx-auto">
      <h1 className="text-3xl font-serif text-center mb-8">Modifier le groupe d'articles</h1>
